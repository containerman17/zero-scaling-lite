...
func checkDownLoop(r *ScalingBackInfoReconciler) {
	...
	ingressMap, err := getIngressMap("10m", r)
...
func getIngressMap(timing string, r *ScalingBackInfoReconciler) (map[string]bool, error) {
	...
	prometheusURL := "http://prometheus-proxy.downscaler-kubebuider-system.svc.cluster.local:9090/api/v1/query?query=" + url.QueryEscape(query)
